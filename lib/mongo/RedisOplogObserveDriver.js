...
/* global Package LocalCollection */
...
export default class RedisOplogObserveDriver {
  ...
  static cursorSupported (cursorDescription, matcher) {
    ...
    // the underscored version of the option is COMPAT with 1.2
    if (options.disableOplog || options._disableOplog) {
return false
}

    // If a field projection option is given, check if it is supported by

...
export default class Mutator {
  static init () {
    ...
    // regardless of your choice, these two packages must passConfigDown
  ...
  static async insert (Originals, data, _config) {
    ...
    if (canUseOriginalMethod(config)) {
      return await Originals.insertAsync.call(
        this,
        data,
    ...
    try {
      const docId = await Originals.insertAsync.call(this, data)
      ...
      if (!protectAgainstRaceConditions(this)) {
        doc = await Originals.findOneAsync.call(this, docId)
      }
      ...
      if (_.isFunction(_config)) {
        Meteor.defer(() => _config.call(this, e))
      } else {
        throw e
      }
    }
  ...
  static async update (Originals, selector, modifier, _config, callback) {
    ...
    if (canUseOriginalMethod(config)) {
      return await Originals.updateAsync.call(
        this,
        selector,
        modifier,
        _config,
    ...
    if (shouldIncludePrevDocument(this)) {
      docs = await this.find(selector, { ...findOptions, fields: {} }).fetchAsync()
    } else {
    ...
    if (config && config.upsert) {
      return await Mutator._handleUpsert.call(
        this,
        Originals,
        selector,
        modifier,
        { _returnObject: false, ...config},
        callback,
        docIds,
        docs
      )
    ...
    const updateSelector = { ...selector, _id: { $in: docIds }}

    try {
      const result = await Originals.updateAsync.call(
        this,
        updateSelector,
        modifier,
        config
  ...
  static async _handleUpsert (
    ...
    try {
      const data = await Originals.updateAsync.call(
        this,
        selector,
        modifier,
        { ...config, _returnObject: true}
      )
  ...
  static async remove (Originals, selector, _config) {
    ...
    if (canUseOriginalMethod(config)) {
      return await Originals.removeAsync.call(
        this,
        selector,
        _.isFunction(_config) ? _config : undefined
      )
    ...
    try {
      const result = await Originals.removeAsync.call(this, removeSelector)

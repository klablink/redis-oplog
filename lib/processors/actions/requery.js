 ...
 */
export default async function (observableCollection, newCommer, event, modifiedFields) {
  const { store, selector, options } = observableCollection

  const newStore = new MongoIDMap()
  const freshIds = await observableCollection.collection.find(
    selector, { ...options, fields: { _id: 1 } }).fetchAsync()

...
class RedisSubscriptionManager {
  ...
  getAllRedisSubscribers () {
    ...
    for (const channel in this.store) {
  ...
  initializeChannel (channel) {
    ...
    const handler = function (message) {
      self.queue.queueTask(async () => {
        debug(
                    `[RedisSubscriptionManager] Received from redis event: "${JSON.stringify(message)}"`
        )
  ...
  async process (channel, data, fromRedis) {
    ...
    if (fromRedis && data[RedisPipe.UID] === this.uid) {
      debug(
                `[RedisSubscriptionManager] Optimistic event from redis skipped "${data[RedisPipe.EVENT]}" to "${channel}"`
      )
      return
    }
    ...
    const isSynthetic = data[RedisPipe.SYNTHETIC]
  ...
  async getDoc (collection, subscribers, data) {
    ...
    if (fieldsOfInterest === true) {
      ...
      doc = await collection.findOneAsync(doc._id, { fields: fieldsOfInterest })
    }

...
export class MongoIDMap {
  ...
  async compareWith (other, callbacks) {
    // operate on the _internal maps to avoid the overhead of parsing id's.
    ...
    if (leftMap) {
      for (const [key, leftValue] of leftMap) {
        ...
        if (rightValue != null) { callbacks.both && await callbacks.both(this._idParse(key), leftValue, rightValue) } else { callbacks.leftOnly && await callbacks.leftOnly(this._idParse(key), leftValue) }
      }
    }

    if (callbacks.rightOnly && rightMap) {

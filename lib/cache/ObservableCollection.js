...
export default class ObservableCollection {
  ...
  constructor ({ multiplexer, matcher, sorter, cursorDescription }) {
    ...
    this._projectionFn = LocalCollection._compileProjection(projection) // Projection function,
    // result of combining important fields for selector and
  ...
  async changeSynthetic (docId, modifier /*, topLevelFields */) {
    ...
    const storedDoc = this.store.get(docId)
    const oldDoc = EJSON.clone(storedDoc)
  ...
  _getFieldsOfInterest () {
    ...
    // if we have options, we surely have a field array

...
const dispatchEvents = async function (optimistic, collectionName, channels, events) {
  debug(
        `[Dispatcher] Dispatch ${collectionName} event ${
            optimistic ? 'optimistic ' : ''
        } "${JSON.stringify(events)}" to "${channels}"`
  )
...
const dispatchRemove = async function (optimistic, collectionName, channels, docs) {
  const uid = optimistic ? RedisSubscriptionManager.uid : null
...
const dispatchInsert = async function (optimistic, collectionName, channels, doc) {
  const uid = optimistic ? RedisSubscriptionManager.uid : null

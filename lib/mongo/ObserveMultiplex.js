...
class ObserveMultiplexer {
  constructor ({ ordered, onStop = () => {} } = {}) {
    ...
    const self = this
    for (const callbackName of this.callbackNames()) {
  ...
  async _applyCallback (callbackName, args) {
    ...
    await this._queue[runType](async function () {
      // If we stopped in the meantime, do nothing.
      if (!self._handles) { return }

      // First, apply the change to the cache.
      await self._cache.applyChange[callbackName].apply(null, EJSON.clone(args))
      ...
      for (const handleId of Object.keys(self._handles)) {
        const handle = self._handles && self._handles[handleId]
        if (!handle) return
  ...
  // flush the queue afterward to ensure that the callbacks get out.
...
export { ObserveMultiplexer, ObserveHandle }
